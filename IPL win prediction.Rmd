...
```{r}
inning_score<- deliveries %>% group_by(match_id, inning) %>% summarise(total_runs = sum(total_runs)) %>% ungroup() %>% filter(inning == 1)
head(inning_score)
```
```{r}
...
```
```{r}
if (!"BowlingTeam" %in% colnames(final)) {
  final$BowlingTeam <- NA
}
if (!"BattingTeam" %in% colnames(final)) {
  final$BattingTeam <- NA
}
...
# Create training indices (70% training, 30% testing)
...
X_train <- as.data.frame(X_train)  # Convert to data frame if not already
y_train <- as.factor(y_train)      # Ensure y_train is a factor


sum(is.na(X_train))  # Count missing values in X_train
...
confusion_matrix_1 <- confusionMatrix(y_pred, as.factor(y_test))
...
ggplot(data = conf_matrix_table, aes(x = Actual, y = Predicted, fill = Freq)) + geom_tile() + geom_text(aes(label = Freq), color = "white", size = 5) + scale_fill_gradient(low = "lightblue", high = "blue") + theme_minimal() + labs(title = "Confusion Matrix", x = "Actual", y = "Predicted") + theme(plot.title = element_text(hjust = 0.5))
```
