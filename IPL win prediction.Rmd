...
```{r}
matches <- matches %>%
  mutate(WinningTeam = ifelse(winner == "N", team2, winner))
...
# Select the top 10 teams by the number of wins
top_teams <- wins %>%
  top_n(10, n)

# Create the bar plot using ggplot2
ggplot(top_teams, aes(x = reorder(WinningTeam, -n), y = n)) +
  geom_bar(stat = "identity", aes(fill = WinningTeam)) +
  labs(title = "Top 10 Best Performing Teams",
       x = "Team",
       y = "Number of Wins") +  scale_fill_brewer(palette = "Set3") +
  theme_minimal() +
...
```{r}
matches <- matches %>%
  mutate(WinningTeam = ifelse(winner == "N", team1, winner))

# Count the number of wins for each team
wins <- matches %>%
  count(WinningTeam) %>%
  arrange(n)  # Sort by the least number of wins

# Select the bottom 10 teams by the number of wins
least_teams <- wins %>%
  top_n(-10, n)

# Create the bar plot for the least performing teams using ggplot2
ggplot(least_teams, aes(x = reorder(WinningTeam, n), y = n)) +
  geom_bar(stat = "identity", fill = "lightcoral", color = "black") +
  labs(title = "Top 10 Least Performing Teams",
       x = "Team",
       y = "Number of Wins") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
matches <- read.csv("C:/Users/omhas/Documents/IPL Project/Dataset/matches.csv")
matches
matches <- matches %>% select(-city, -id, -method) %>% na.omit()

# Create SEASON_INT and SEASON_END_INT columns
matches <- matches %>%
  mutate(
    SEASON_INT = as.numeric(substr(season, 1, 4)),
    SEASON_END_INT = ifelse(nchar(season) > 5, as.numeric(paste0('20', substr(season, 6, 7))), SEASON_INT)
  )

# Convert 'Date' column to Date type
matches$Date <- as.Date(matches$date)

# Group by 'Season' and select the last row for each group based on the date
...
# Plot using ggplot2
ggplot(winning_teams, aes(x = SEASON_END_INT, y = winner, fill = as.factor(SEASON_END_INT))) +
  geom_col() +
  labs(y = "IPL Teams", x = "Seasons", title = "IPL Winning Teams by Year") +
...
head(final_df)
final_df <- final_df %>%
  mutate(Batting_Avg = ifelse(is.infinite(Batting_Avg), NA, Batting_Avg)) %>%  # Replace Inf with NA
  filter(!is.na(Batting_Avg))

final_df <- final_df %>%
  mutate(Bowling_Eco = ifelse(is.infinite(Bowling_Eco), NA, Bowling_Eco)) %>%  # Replace Inf with NA
  filter(!is.na(Bowling_Eco))

final_df <- final_df %>%
  mutate(Bowl_Strike_Rate = ifelse(is.infinite(Bowl_Strike_Rate), NA, Bowl_Strike_Rate)) %>%  # Replace Inf with NA
  filter(!is.na(Bowl_Strike_Rate))
...
# Remove NA values and those outside the range of -1 to 1
melted_correlation <- melted_correlation %>%
    filter(!is.na(value) & value >= -1 & value <= 1)
...
heatmap_plot <- ggplot(data = melted_correlation, aes(x = Var1, y = Var2, fill = value)) +
  ...
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       ...
                       name = "Correlation") +  # Define color gradient
  geom_text(aes(label = round(value, 2)), color = "black", size = 2) +  # Add correlation values on tiles
  theme_minimal(base_size = 10) +  # Use a minimal theme with larger base size
