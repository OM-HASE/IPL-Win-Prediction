...
y_pred_nb <- predict(naive_bayes_model, newdata = X_test)
...
get_roc_data <- function(roc_obj, model_name) {
  data.frame(
...
# Correct the labels in the legend and ensure correct order

# Combine the ROC data frames in the correct sequence
roc_combined <- rbind(
  get_roc_data(roc_nb, "Naive Bayes"),
  get_roc_data(roc_decision_tree, "Decision Tree"),
  get_roc_data(roc_logistic, "Logistic Regression"),
  get_roc_data(roc_rf, "Random Forest")
)
...
ggplot(roc_combined, aes(x = FPR, y = TPR, color = Model)) +
  ...
  theme_minimal() +
  labs(title = "AUC-ROC Curves", x = "False Positive Rate (FPR)", y = "True Positive Rate (TPR)") +
  ...
  annotate("text", x = 0.8, y = 0.2, label = paste("AUC (NB):", round(auc(roc_nb), 3)), color = "red") +
  annotate("text", x = 0.8, y = 0.15, label = paste("AUC (DT):", round(auc(roc_decision_tree), 3)), color = "blue") +
  annotate("text", x = 0.8, y = 0.1, label = paste("AUC (LR):", round(auc(roc_logistic), 3)), color = "purple") +
  annotate("text", x = 0.8, y = 0.05, label = paste("AUC (RF):", round(auc(roc_rf), 3)), color = "green")
